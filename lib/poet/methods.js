...
function init (poet, callback) {
  ...
  //re-init to avoid garbage on delete
  poet.posts = {}

  if (options.useDb){

      var promise = db.getPostsFromDb(function (err, posts) {
          if (err)
              return null

          // Generate a collection of promises that resolve to post objects
          var collection = posts.reduce(function (coll, postObj) {

              var template = utils.getTemplate(poet.templates, '.md');
              ...
              var post = utils.createPostFromDb(postObj, options)

              all([template(post.content), template(post.preview)]).then(function (contents) {
      ...
      var promise = utils.getPostPaths(options.posts).then(function (files) {
        ...
        var collection = files.reduce(function (coll, file) {
          ...
          // Do the templating and adding to poet instance
          // here for access to the file name
          var post = utils.createPostFromFile(file, options).then(function (post) {
