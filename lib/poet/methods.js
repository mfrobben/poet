...
function init (poet, callback) {
  ...
  if (options.useDb){
      ...
      var promise = db.getPostsFromDb().then(function (posts) {
          // Generate a collection of promises that resolve to post objects
          posts.map(function (postObj) {
              var template = poet.templates[postObj.template]
          ...
          // Clear out the cached sorted posts, tags, categories, as this point they could have changed from new posts
          clearCache(poet);
          return poet;
      });
  } else {
